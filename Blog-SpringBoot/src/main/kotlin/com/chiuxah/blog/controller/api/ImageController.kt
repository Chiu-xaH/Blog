...
class ImageController {
    ...
    fun uploadImage(
        ...
        val types = when(type) {
            ImageType.ARTICLE_PHOTO.name -> ImageType.ARTICLE_PHOTO
            ImageType.USER_PHOTO.name -> ImageType.USER_PHOTO
            ImageType.COMMENT_PHOTO.name -> ImageType.COMMENT_PHOTO
    ...
    fun uploadUserPhoto(
        ...
        val responseBody = uploadImage(image, ImageType.USER_PHOTO.name,request)
        return if(isSuccessResponse(responseBody)) {
            // 将响应URL设置为用户信息的photo
            val data = jsonToMap(responseBody)["data"] ?: return ControllerUtils.CONTROLLER_ERROR_RESPONSE
